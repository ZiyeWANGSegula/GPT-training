...
const TABLE_HEAD = [
  ...
  { id: 'lib', label: 'Facility reference', align: 'left' },
...
function applyFilter({ inputData, comparator, filterSearch, filterRole, filterFirstDate,
  ...
  if (filterSearch) {
    inputData = inputData.filter(
      (resource) => 
      resource.id.toString().indexOf(filterSearch) !== -1 ||
      resource.lib.toLowerCase().indexOf(filterSearch.toLowerCase()) !== -1 ||
      resource.cat.toLowerCase().indexOf(filterSearch.toLowerCase()) !== -1 ||
  ...
  if (filterRole !== 'ALL') {
    inputData = inputData.filter((resource) => resource.cat === filterRole);
  }
